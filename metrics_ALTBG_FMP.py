...
def get_metrics():
    ...
    try:
        ...
        # Dates for 2-day history (buffer for weekends/holidays)
        end_date = today.strftime('%Y-%m-%d')
        start_hist = (today - timedelta(days=3)).strftime('%Y-%m-%d')

        # Fetch historical prices for BTC and ALCPB (2 days)
        btc_hist_data = fetch_fmp_data('historical-price-full', {'symbol': 'BTC-EUR', 'from': start_hist, 'to': end_date})
        ...
        if btc_hist_data and 'historical' in btc_hist_data and len(btc_hist_data['historical']) >= 2:
            hist_btc = btc_hist_data['historical']
            btc_price = float(hist_btc[-1]['close'])
            btc_price_yesterday = float(hist_btc[-2]['close'])
        ...
        shares_data = fetch_fmp_data('historical-shares-outstanding', {'symbol': 'ALCPB.PA', 'limit': 40})
        shares_now_out = None
        ...
        return jsonify({
            ...
            "btc_torque": round(btc_torque, 4) if btc_torque else None,  # Added rounding for consistency
